...
public class EssbaseCalculationService {
	...
	private static CompletableFuture<AnalyticExportFile> exportHCWithFixStatement(EssbaseCube cube,String strHome, String str ) {
		...
		try {
			exportHC = cube.export(f -> f
				.fileName("actalc_4regalc_" + str.toLowerCase() + ".txt")
				.addFixStatement(fix)
	...
	private static CompletableFuture<AnalyticExportFile> exportWithFixStatement(EssbaseCube cube,String strHome, String str ) {
		String fix = "FIX (@RELATIVE(\"Company\", 0), @RELATIVE(\"Funding Type Total\", 0),@RELATIVE(\"Fixed Pool Total\", 0),@RELATIVE(\"" + str +"\", 0),@RELATIVE(\"Product Total\", 0),@RELATIVE(\"" + str + "\", 0), " + Helpers.translateMonthNumber(Def.CP) + ")"; 
